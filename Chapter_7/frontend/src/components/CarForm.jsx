...
const schema = z.object({
    brand: z.string().min(2, 'Brand must contain at least 2 letters').max(20, 'Brand cannot exceed 20 chars'),
    make: z.string().min(1, 'Car model must be at least 1 character long').max(20, 'Model cannot exceed 20 chars'),
...
const CarForm = () => {
    ...
    let formArray = [
        {name: "brand", type: "text", error: errors.brand},
    ...
    const onSubmit = async (data) => {
        ...
        if (result.ok) {
            ...
            setMessage(JSON.stringify(json));		// print json responce in case of errors
            navigate("/");
        }
    };
                    ...
                    {formArray.map((item, index) => (
                        <InputField key={index}
                                    props={{
                                        name: item.name,
                                        type: item.type,
                                        error: item.error, ...register(item.name),
                                    }}
                        />
                    ))}
                    <div className="flex items-center justify-between">
                        <button
                            className="bg-gray-900 hover:bg-gray-700 text-white w-full font-bold py-2 px-4 rounded
